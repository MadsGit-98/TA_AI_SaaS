...
document.addEventListener('DOMContentLoaded', function() {
    ...
    /**
     * Process a newly selected resume file: validate it locally, update file UI, and start server-side duplication validation.
     *
     * Clears previous file messages, performs local size/extension validation, updates displayed file name and size,
     * reveals the file info area, and triggers the asynchronous server-side validation for duplicates or other server checks.
     *
     * @param {File} file - The selected file object (expected resume file) to validate and display.
     */
    ...
    /**
     * Validate a selected file's size and extension against allowed limits.
     * @param {File} file - The file to validate.
     * @returns {{valid: boolean, error?: string}} `valid: true` if the file meets size and extension requirements; otherwise `valid: false` and `error` contains a user-facing message explaining the failure.
     */
    ...
    /**
     * Performs a debounced server-side validation and duplication check for the provided resume file, updating validation state and related UI.
     *
     * Sends the file and job listing ID to the server, then marks the file as validated on success or displays appropriate error/duplicate messages on failure.
     * @param {File} file - The resume file to validate against the job listing.
     */
    ...
    /**
     * Clear the currently selected resume and reset related UI and validation state.
     *
     * Clears the stored file and server-validation flag, empties the resume file input,
     * hides file info UI, clears file-related messages, and re-evaluates overall form validity.
     */
    function removeFile() {
        selectedFile = null;
        fileValidated = false;
    ...
    /**
     * Validate a single form control and update its visible validation state.
     *
     * If an element with id `<field.name>-error` exists, the function updates its text
     * with a required-field message when the control is empty or with an email-specific
     * message when an email input has a type mismatch. It also toggles CSS classes
     * to indicate error state on the control.
     *
     * @param {HTMLElement} field - The form control to validate (e.g., input, textarea, select).
     * @returns {boolean} `true` if the control passes HTML5 validity checks, `false` otherwise.
     */
    ...
    /**
     * Validate the application form and mark invalid fields.
     *
     * Performs validation of the country code selector, all required fields
     * (text inputs, radio groups, checkbox groups, select dropdowns, and file uploads),
     * ensures a resume is selected and has passed server-side validation, and sets
     * per-field error messages for any failures. Also synchronizes the hidden
     * country code input with the selected country when present.
     *
     * @returns {boolean} `true` if all validations pass and the form is ready to submit, `false` otherwise.
     */
    ...
    /**
     * Update the submit button enabled state based on whether all required form fields are filled and the selected resume file has passed server validation.
     *
     * Checks each required field in the form and considers:
     * - radio groups: at least one option checked;
     * - checkbox groups: at least one box checked;
     * - select elements: value is not empty after trimming;
     * - file inputs: at least one file selected;
     * - other inputs/textareas: value is not empty after trimming.
     *
     * If a submit button is present, its disabled state is set to `false` only when all required fields are filled, a resume is selected (`selectedFile`), and the resume has passed server validation (`fileValidated`); otherwise it is set to `true`.
     */
    ...
    /**
     * Submit the application form to the server, including resume and per-question files, and handle the resulting UI updates.
     *
     * Collects form fields and screening answers into FormData, appends any resume and question file uploads, sends the submission to the backend, and updates the page based on the response. On success it shows a success message and redirects; on duplicate detection it shows appropriate duplicate messages/modals; on validation errors it highlights fields and scrolls to the first error; on network or server errors it restores the submit state and displays a generic error message.
     */
     ...
     * @returns {string} A formatted size using `B`, `KB`, or `MB`; KB/MB are rounded to one decimal place.
     */
    ...
    /**
     * Retrieve the CSRF token from the page's meta tag.
     * @returns {string} The CSRF token string from the `meta[name="csrf-token"]` tag, or an empty string if the tag is not present.
     */
    ...
    /**
     * Display a file-related error message and hide any file success message.
    ...
    /**
     * Displays a file upload success message in the UI and hides any visible file error.
     * @param {string} message - The success text to show in the file upload success element.
     */
    ...
    /**
     * Clear and hide file-related error and success messages in the UI.
     *
     * If present, removes the error message text and hides the error and success elements.
     */
    ...
    /**
     * Clear all form error indicators and reset input border styling.
     *
     * Clears text and hides elements with the `.error-message` class, removes the
     * red error border class from all inputs, textareas, and selects and restores
     * the default border class, and clears any file-related messages.
     */
    ...
    /**
     * Display the duplicate modal with the provided message.
     * @param {string} message - Text to show inside the duplicate modal.
     */
    ...
    /**
     * Displays a success screen with the provided message and redirects to the homepage after a short countdown.
...
/**
 * Hides the duplicate application modal dialog.
 *
 * This closes the modal shown for duplicate resume/email/phone warnings by making the dialog not visible.
 */
function closeModal() {
    const modal = document.getElementById('duplicate-modal');
...
/**
 * Sets up per-question file upload UI interactions for elements with IDs beginning `file-upload-area-`.
 *
 * Binds a click on each upload area to trigger its nested file input and updates the corresponding
 * `file-name-<id>` text and reveals the `file-info-<id>` element when a file is chosen.
 */
...
/**
 * Clear the selected file for a per-question file upload and hide its file info UI.
 * @param {string|number} questionId - Identifier for the question; used to locate the file input named `question_file_<questionId>` and the file info element with id `file-info-<questionId>`.
 */
