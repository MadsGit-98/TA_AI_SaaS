...
class ApplicationFlowE2ETest(LiveServerTestCase):
    ...
    def setUpClass(cls):
        """
        Prepare a headless Chrome Selenium WebDriver for the test class.
        
        Configures Chrome options (headless, no-sandbox, disable GPU and shared memory usage, window size 1920x1080),
        instantiates the WebDriver as `cls.selenium`, and sets an implicit wait of 10 seconds.
        """
        super().setUpClass()
    ...
    def test_application_form_loads_successfully(self):
        """
        Verify the application page loads and displays the job title, a portion of the job description, and each required skill.
        """
    ...
    def test_application_form_validation(self):
        """
        Verify the application form's client-side validation prevents submission when required fields are incomplete.
        
        Asserts the submit button is disabled before any input and remains disabled after entering only the first name.
        """
        self.selenium.get(self.application_url)
    ...
    def test_application_form_mobile_responsive(self):
        """
        Verify the application form adapts to a mobile viewport and presents a stacked single-column layout.
        
        Loads the application page at a mobile-sized viewport (375x812), asserts the main application form is visible, and if form rows exist checks their CSS grid template columns indicate a single-column (stacked) layout by containing "1fr".
        """
        # Set mobile viewport
