...
class ApplicationStatusSecurityTests(TestCase):
    ...
    def setUp(self):
        """
        Prepare test fixtures for ApplicationStatusSecurityTests.
        
        Creates an APIClient instance and test data: a user, a JobListing (active, starts yesterday, expires in 30 days), and an Applicant associated with that job listing whose status is 'submitted'. These objects are assigned to self.client, self.user, self.job_listing, and self.applicant for use in tests.
        """
