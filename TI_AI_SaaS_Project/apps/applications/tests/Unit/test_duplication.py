...
class DuplicationServiceTest(TestCase):
    ...
    def setUp(self):
        """
        Create test fixtures: a user, a job listing, and an existing applicant used by the tests.
        
        Sets up:
        - a test user with username 'testuser' and email 'test@example.com';
        - a JobListing tied to that user (title 'Test Developer', one required skill, start and expiration dates);
        - an Applicant linked to the JobListing with email 'john@example.com', phone '+12025551234', and resume_file_hash 'abc123def456'.
        """
    ...
    def test_check_resume_duplicate_not_found(self):
        """
        Verifies that a resume hash different from existing applicants is not reported as a duplicate for the job listing.
        """
        is_duplicate = DuplicationService.check_resume_duplicate(
            self.job_listing,
            'different_hash'
