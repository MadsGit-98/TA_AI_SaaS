...
class EmailDeliveryIntegrationTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare common test fixtures for the email delivery integration tests.
        
        Creates a Django test client and the persistent objects used by the tests: a test user, a JobListing populated with sample fields, and a ScreeningQuestion linked to that JobListing.
        """
        self.client = Client()
    ...
    def create_valid_resume(self):
        """
        Create a minimal valid PDF resume file suitable for upload in tests.
        
        The generated PDF contains a minimal PDF header and is padded to be at least 51 KB to satisfy file-size validations. The returned file is named "resume.pdf" and has content type "application/pdf".
        
        Returns:
            SimpleUploadedFile: An in-memory uploaded file containing the PDF resume.
        """
    ...
    def test_email_contains_correct_job_title(self):
        """
        Verify that submitting an application creates an Applicant associated with the correct job listing.
        
        This integration test submits a multipart application (including a resume and screening answers) and asserts the response is 201. Because email delivery is performed asynchronously, the test does not inspect email content; instead it verifies an Applicant exists with the submitted email and that the Applicant.job_listing matches the expected JobListing.
        """
