...
class ApplicationSubmissionIntegrationTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare test fixtures for application submission integration tests.
        
        Creates a test HTTP client, a test user, an active job listing, a screening question linked to that listing, and a sample `valid_application_data` payload (including screening answer) used by the tests.
        """
        self.client = Client()
    ...
    def create_valid_resume(self):
        """
        Create a minimal valid PDF file padded to at least 50 KB and return it as an uploaded file suitable for multipart test requests.
        
        The returned file is named 'resume.pdf' and has content_type 'application/pdf'. The PDF contains a minimal header and enough padding to ensure its size is at least 50 KB.
        
        Returns:
            SimpleUploadedFile: An in-memory uploaded file containing the generated PDF.
        """
    ...
    def test_submit_application_success(self):
        """
        Verifies that submitting a complete application with a valid PDF resume and JSON-encoded screening answers returns a submitted response and persists the applicant and answers.
        
        Sends a multipart POST to /api/applications/ with valid job_listing_id, applicant details, a resume file, and screening_answers. Asserts the response status code is 201, response contains an `id`, `status` equals "submitted", and the success message is present. Confirms an Applicant record is created with the provided name and email and that a corresponding ApplicationAnswer is stored linking the applicant to the screening question with the provided answer text.
        """
