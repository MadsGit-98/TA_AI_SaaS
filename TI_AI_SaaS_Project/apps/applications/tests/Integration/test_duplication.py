...
class DuplicationIntegrationTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare test fixtures used by duplication integration tests.
        
        Creates a test HTTP client and a user, then creates a JobListing associated with that user and an existing Applicant tied to the job listing with sample contact and resume data so tests can exercise duplicate-detection flows.
        """
        self.client = Client()
    ...
    def create_resume(self, content_size=51):
        """
        Create a SimpleUploadedFile containing a minimal PDF-like resume of the requested size.
        
        Parameters:
            content_size (int): Desired file size in kilobytes (KB). Defaults to 51.
        
        Returns:
            SimpleUploadedFile: An uploaded file named 'resume.pdf' with MIME type 'application/pdf' and a payload padded to approximately `content_size` KB.
        """
    ...
    def test_submit_duplicate_email(self):
        """
        Submit an application using an email that matches an existing applicant to verify duplicate detection.
        
        Verifies the API responds with HTTP 409, marks the submission as invalid, sets `checks['duplicate_detected']` to `True`, and returns exactly one error with code `'duplicate_detected'` and a privacy-preserving message referencing similar contact information.
        """
        resume = self.create_resume()
