...
(function() {
    ...
    /**
     * Attach a click handler to the reporting page toggle button to switch between the table and comparison views and update the button label.
     *
     * If the toggle button or either view element is not present, the function performs no action.
     */
    ...
    /**
     * Set up event handlers for the results list page: filter application, result detail modal controls, and refresh.
     *
     * - Applies filters: when the "apply-filters" control is activated, navigates to the current path with
     *   non-empty `category`, `min_score`, and `max_score` values added as query parameters.
     * - Detail view: attaches click handlers to elements with class `btn-view-detail` to open a result's detail view
     *   by calling `openResultDetail` with the element's `data-result-id`.
     * - Modal close: binds the `.modal-close` control to hide the `#result-detail-modal` element when clicked.
     * - Refresh: binds `#refresh-results` to reload the page when clicked.
     */
    ...
    /**
     * Fetches detail data for the given result and displays it in the result detail modal.
     * If the API returns an error or the network request fails, displays an error message in the modal.
     * @param {string} resultId - Identifier of the result to fetch and display.
     */
    ...
    /**
     * Render HTML for a result detail modal from API result data.
     * @param {Object} data - Result payload containing applicant, scores, screening_questions, and status.
     * @returns {string} The HTML markup for the result detail modal.
     */
    function buildDetailContent(data) {
        /**
         * Normalize a score value for display: return 'N/A' for missing values or an escaped string otherwise.
         * @param {*} value - The score value to normalize; null, undefined, or empty string are treated as missing.
         * @returns {string} `'N/A'` if the value is null, undefined, or an empty string, otherwise the HTML-escaped string form of the value.
         */
    ...
    /**
     * Hide the result detail modal when the click target is the modal overlay.
     * @param {Event} event - Click event whose target is checked to determine if the modal was clicked.
     */
...
(function() {
    ...
    /**
     * Initializes accordion behavior for elements with the `accordion-header` class.
     *
     * Attaches click handlers that close all accordion sections and then, if the clicked header was not already active and its matching content exists, mark the header active, set `aria-expanded="true"`, and show the associated content element whose id is `accordion-` plus the header's `data-accordion` value. When closing, headers have their active state removed and `aria-expanded` set to `"false"`, and all `.accordion-content` elements are hidden.
     */
...
(function() {
    ...
    /**
     * Controls and displays a terminal-style loading indicator and progress UI for long-running analysis jobs.
     *
     * Initializes DOM element references, internal state (job id, polling, displayed milestones), and binds the cancel button to the instance's cancelAnalysis method when present.
     * @constructor
     */
