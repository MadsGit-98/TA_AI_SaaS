...
class AnalysisResultsAPIIntegrationTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare full integration test state: client, users/profiles, authenticated session, an expired job listing, 25 applicants, and corresponding AI analysis results.
        
        Sets up:
        - a Django test client and an authenticated session via the real login endpoint (access token cookie present);
        - two users with UserProfile records (one job owner, one non-owner) for RBAC testing;
        - one expired JobListing owned by the primary user;
        - 25 Applicant records associated with the job; and
        - one AIAnalysisResult per applicant with varying metric scores (overall_score ranges from 40 to 64), derived category, status set to 'Analyzed', and per-metric and overall justifications.
        """
        self.client = Client()
    ...
    def _get_category(self, score):
        """
        Map a numeric overall score to a match category label.
        
        Parameters:
            score (int | float): Overall numeric score (expected 0–100).
        
        Returns:
            str: One of 'Best Match' (score >= 90), 'Good Match' (score >= 70), 'Partial Match' (score >= 50), or 'Mismatched' (score < 50).
        """
        if score >= 90:
            return 'Best Match'
        elif score >= 70:
    ...
    def test_filter_by_score_range(self):
        """
        Verify the API returns analysis results whose `overall_score` is within the specified inclusive range.
        
        Sends a request with `min_score` and `max_score` query parameters, asserts a 200 response, and checks that every returned result has `overall_score` >= 50 and <= 60.
        """
        url = f'/api/analysis/jobs/{self.job.id}/analysis/results/'
