...
class AnalysisThrottleIntegrationTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare a fresh test client and reset throttle-related cache before each test.
        
        Initializes self.client with a new Django test Client and clears the cache to ensure throttle state does not carry over between tests.
        """
        self.client = Client()
    ...
    def _login(self, ip_address='127.0.0.1'):
        """
        Authenticate the test user via the API login endpoint.
        
        Parameters:
            ip_address (str): IP address to include with the login request (default '127.0.0.1').
        
        Returns:
            bool: `True` if the login response status code is 200, `False` otherwise.
        """
