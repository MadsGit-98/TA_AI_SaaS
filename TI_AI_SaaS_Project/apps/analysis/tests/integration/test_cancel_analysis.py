...
class CancelAnalysisAPIIntegrationTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare integration test fixtures: initialize a Django test client, create an owner and a non-owner user with required UserProfile entries, authenticate the owner via the real login endpoint (populating JWT cookies), and create an expired JobListing owned by the test user.
        """
        self.client = Client()
