...
class InitiateAnalysisAPIIntegrationTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare integration test fixtures: HTTP client, two users with RBAC profiles, authenticate the owner via the real login endpoint, and create an expired job listing.
        
        Sets up:
        - a Django test Client;
        - an owner user and a non-owner user, each with a UserProfile enabling talent acquisition permissions;
        - authenticates the owner through the API and asserts a successful login with an access_token cookie;
        - an expired, inactive JobListing created by the owner.
        """
        self.client = Client()
    ...
    def tearDown(self):
        """
        Clear the global cache to reset rate-limiting/throttling counters between tests.
        """
