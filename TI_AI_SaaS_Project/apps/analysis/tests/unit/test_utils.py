...
class RedisLockUtilitiesTest(TestCase):
    ...
    def test_acquire_analysis_lock_success(self, mock_redis):
        ...
        # Should return owner_id (UUID string) when successful
        ...
        self.assertEqual(call_args[0][0], 'analysis_lock:test-job-id')
        self.assertEqual(call_args[1]['nx'], True)
        self.assertEqual(call_args[1]['ex'], 300)
    ...
    def test_release_analysis_lock(self, mock_redis):
        """Test releasing lock with owner verification."""
        mock_conn = MagicMock()
        mock_redis.return_value = mock_conn

        from services.ai_analysis_service import release_analysis_lock

        release_analysis_lock('test-job-id', 'test-owner-id')

        # Verify eval was called for atomic compare-and-delete
