...
class DecisionNodeTest(TestCase):
    ...
    def setUp(self):
        """
        Create a test user and a job listing used by the tests.
        
        Creates a User with username "testuser" and a JobListing titled "Test Job" whose fields are:
        required_skills ['Python'], required_experience 3, job_level 'Entry', start_date 30 days ago,
        expiration_date 1 day ago, status 'Inactive', and created_by set to the test user.
        """
...
class BulkPersistNodeTest(TestCase):
    ...
    def setUp(self):
        """
        Create a test user, a job listing, and ten applicants used by the unit tests.
        
        The job listing is created with required_skills ['Python'], required_experience 3, job_level 'Entry',
        start_date set to 30 days ago, expiration_date set to 1 day ago, status 'Inactive', and the test user
        as creator. Ten Applicant records are created and associated with the job listing; each applicant has
        distinct first/last names, email, phone, resume file/hash, and a parsed resume text of 'Test resume'.
        """
        self.user = User.objects.create_user(
            username='testuser',
            email='tas@example.com',
...
class MapWorkersNodeTest(TestCase):
    ...
    def setUp(self):
        """
        Create a test user, an inactive/expired job listing, and five applicant records for use by the tests.
        
        The job listing is populated with required skills, experience, level, start and expiration dates (set to be expired), status 'Inactive', and is owned by the created user. Five applicants are created and associated with that job; each applicant has basic contact fields, a resume file/hash, and resume_parsed_text.
        """
        self.user = User.objects.create_user(
            username='testuser',
            email='tas@example.com',
...
class EliminationNodeTest(TestCase):
    ...
    def setUp(self):
        """
        Prepare common test fixtures used across node unit tests.
        
        Creates:
        - self.relevant_classified_data: a classified resume representing a software engineer (experience, education, skills, supplemental).
        - self.irrelevant_classified_data: a classified resume representing an accountant.
        - self.job_requirements: a software engineer job specification (title, description, required_skills, required_experience, job_level).
        - self.applicant: a MagicMock applicant with id 'test-applicant-id' for use in worker and supervisor node tests.
        """
