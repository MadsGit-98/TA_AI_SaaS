...
class InputValidationSecurityTest(TestCase):
    ...
    def setUpClass(cls):
        """
        Prepare shared test fixtures for the class: create a test user and profile, a job listing, an applicant, and an AIAnalysisResult, and attach them to class attributes for use by all tests.
         
        Creates the following class attributes:
        - cls.user: Django user acting as the job owner.
        - cls.job: JobListing instance used by tests.
        - cls.applicant: Applicant linked to `cls.job`.
        - cls.analysis_result: AIAnalysisResult linked to `cls.applicant` and `cls.job`.
        
        This setup runs once for the entire TestCase class.
        """
        super().setUpClass()
    ...
    def setUp(self):
        """
        Prepare a fresh Django test client and clear the cache before each test.
        """
        self.client = Client()
        cache.clear()
    ...
    def _login(self):
        """
        Attempt to authenticate the test client using the test user's credentials.
    ...
    def test_xss_in_justification_fields(self):
        """
        Verify the results endpoint returns a valid response when justification fields contain XSS payloads.
        
        Creates a distinct Applicant and AIAnalysisResult for each XSS payload, requests the specific result endpoint, asserts the response is HTTP 200, and removes the created records.
        """
        if not self._login():
            self.fail("Login failed")
    ...
    def test_invalid_uuid_format_rejected(self):
        """
        Verify that requests with malformed job_id UUIDs are rejected with a client error (400) or not found (404) instead of causing a server error (500).
        
        The test sends GET requests to the job analysis status endpoint using a variety of invalid UUID strings and asserts the response status is 400 or 404.
        """
        if not self._login():
            self.fail("Login failed")
