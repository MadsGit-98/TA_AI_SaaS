...
class RedisSecurityTest(TestCase):
    ...
    def setUpClass(cls):
        """
        Initialize shared test fixtures (user, user profile, and job listing) for the test class.
        
        Creates the following class attributes for use by tests:
        - `user`: a test User instance
        - `job`: a test JobListing instance
        Also creates a UserProfile linked to `user`. This setup runs once before any tests in the class.
        """
        super().setUpClass()
    ...
    def _get_redis_client(self):
        """
        Get a Redis client for tests and skip the current test if Redis is unavailable.
        
        Returns:
            redis_client: An active Redis client instance obtained from the test environment.
        """
        try:
            r = get_redis_client()
    ...
    def test_cache_key_format_consistent(self):
        """
        Verify that Redis keys for analysis use consistent prefixes and include the job_id.
        
        Asserts that:
        - lock keys start with "analysis_lock:"
        - cancellation keys start with "analysis_cancel:"
        - progress keys start with "analysis_progress:"
        and that each key contains the generated job_id.
        """
        job_id = str(uuid.uuid4())
