...
class ResumeParserService:
    ...
    def extract_text_from_pdf(file_content: bytes) -> str:
        """
        Extract all textual content from a PDF's pages into a single string.
        
        Parameters:
            file_content (bytes): Raw bytes of the PDF file.
        
        Returns:
            extracted_text (str): The concatenated text from all pages, separated by newline characters and trimmed of trailing whitespace.
        """
    ...
    def extract_text_from_docx(file_content: bytes) -> str:
        """
        Extracts readable text from a DOCX file's paragraphs and table cells.
        
        Parameters:
            file_content (bytes): Raw DOCX file bytes.
        
        Returns:
            str: Concatenated text extracted from paragraphs and table cells, separated by newlines; duplicate table cell values are included only once and the result is trimmed of leading/trailing whitespace.
        """
...
class ConfidentialInfoFilter:
    ...
    def redact(cls, text: str) -> str:
        """
        Redact personally identifiable and confidential information from resume text.
        
        Performs redaction for emails, phone numbers, addresses, Social Security numbers, and dates of birth in a defined order to avoid false matches (addresses are redacted before SSNs to prevent ZIP+4 values from being mistaken for SSNs).
        
        Parameters:
            text (str): Raw parsed resume text to redact.
        
        Returns:
            str: The input text with PII replaced by redaction tokens (e.g., "[EMAIL_REDACTED]").
        """
    ...
    def _redact_emails(cls, text: str) -> str:
        """
        Redacts email addresses in the provided text.
        
        Returns:
            The input text with all email addresses replaced by "[EMAIL_REDACTED]".
        """
    ...
    def _redact_phones(cls, text: str) -> str:
        """
        Redact phone numbers found in the input text, validating numeric candidates with the phonenumbers library.
        
        Replaces matched phone-like substrings with "[PHONE_REDACTED]" for:
        - alphanumeric numbers (e.g., "1-800-FLOWERS"),
        - strings that phonenumbers.is_possible_number recognizes as possible numbers (tries US parsing and an international attempt if needed).
        
        If a matched substring cannot be validated as a possible phone number, it is left unchanged.
        
        Parameters:
            text (str): Input text potentially containing phone numbers.
        
        Returns:
            str: Text with validated phone numbers and alphanumeric phone-like matches replaced by "[PHONE_REDACTED]".
        """
        def replace_phone(match):
            """
            Determine whether a regex match represents a phone number that should be redacted and return either the redaction token or the original match.
            
            Parameters:
                match (re.Match): A regex match object for a candidate phone substring.
            
            Returns:
                str: "[PHONE_REDACTED]" if the matched text is alphanumeric (e.g., "1-800-FLOWERS") or is recognized as a possible phone number by the phonenumbers library; otherwise the original matched text.
            """
    ...
    def _redact_ssn(cls, text: str) -> str:
        """
        Redacts U.S. Social Security numbers in the given text.
        
        Replaces any substring matching the class's SSN_PATTERN with "[SSN_REDACTED]".
        
        Parameters:
            text (str): Input text to redact.
        
        Returns:
            str: Text with Social Security numbers replaced by "[SSN_REDACTED]".
        """
    ...
    def _redact_dates_of_birth(cls, text: str) -> str:
        """
        Replace recognized date-of-birth occurrences with "[DOB_REDACTED]".
        
        Returns:
            str: The input text with date-of-birth values replaced by "[DOB_REDACTED]".
        """
    ...
    def _redact_addresses(cls, text: str) -> str:
        """
        Redacts US and Canadian street addresses, PO Boxes, and rural routes from the given text.
        
        Replaces matched address patterns with the token "[ADDRESS_REDACTED]".
        
        Parameters:
            text (str): Input text that may contain addresses.
        
        Returns:
            str: The input text with addresses replaced by "[ADDRESS_REDACTED]".
        """
